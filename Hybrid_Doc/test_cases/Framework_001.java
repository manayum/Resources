...
import app_modules.CheckOutAction;
 
import app_modules.ConfirmationAction;
 
import app_modules.PaymentDetailsAction;
 
import app_modules.ProductSelectAction;
 
import app_modules.SignInAction;
 
import app_modules.VerificationAction;
...
public class Framework_001{
  ...
  public void beforeMethod() throws Exception {
		...
		iTestCaseRow = ExcelUtils.getRowContains(sTestCaseName,Constant.ColTestCaseName);
  ...
  public void main() throws Exception {
	  ...
	  try{
		...
		SignInAction.Execute(iTestCaseRow);
		...
		ProductSelectAction.productType(iTestCaseRow);
		...
		ProductSelectAction.productNumber(iTestCaseRow);
		...
		PaymentDetailsAction.execute(iTestCaseRow);
		...
		if(BaseClass.bResult==true){
			...
			ExcelUtils.setCellData("Pass", iTestCaseRow, Constant.ColResult);
	  ...
	  // Below are the steps you may like to perform in case of failed test or any exception faced before ending your test 
		  ...
		  ExcelUtils.setCellData("Fail", iTestCaseRow, Constant.ColResult);
